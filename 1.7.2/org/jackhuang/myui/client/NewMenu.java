...
public class NewMenu extends GuiScreen {
	...
	protected void actionPerformed(GuiButton p_146284_1_) {
		...
		if (p_146284_1_.id == 13) {
			openLink(this.outdate_link, 97);
		} else if (p_146284_1_.id == 23) {
			openLink(MyUI.instance.webiteURL, 96);
		} else if (p_146284_1_.id == 24) {
			openLink(MyUI.instance.contactURL, 95);
		} else if (p_146284_1_.id == 25) {
			openLink(MyUI.instance.qGroupURL, 94);
		}
	...
	public void confirmClicked(boolean par1, int par2) {
		if (par1 && par2 == 12) {
			...
			if(par1)
				try {
					java.awt.Desktop.getDesktop().browse(new URI(myURL[par2]));
				} catch(Throwable t) {
					t.printStackTrace();
				}
	...
	public void drawScreen(int par1, int par2, float par3) {
		...
		String s2 = "Copyright(c) 2014 modded by huangyuhui. All Rights Reserved.";
	...
	protected void mouseClicked(int par1, int par2, int par3) {
		...
		if (par2 < 15) {
			openLink(MyUI.instance.updateURL, 99);
		}
		...
		if ((block - 2) <= par1 && par1 <= (block * 2) && i <= par2
				&& par2 <= (i + 24 * 3 - 4)) {
			openLink(MyUI.instance.announcementPictureURL, 98);
		}
	...
	public void openLink(String url, int index) {
		myURL[index] = url;
		GuiConfirmOpenLink guiconfirmopenlink = new GuiConfirmOpenLink(this, url, 13, true);
